...
class GuardianBLE:
    ...
    async def get_ble_devices(self) -> list:
        ...
        # NEW: allow both old "IGEB" and new "IGE-XXXX" names
        valid_prefixes = ("IGEB", "IGE", "IGE-")
        ...
        for _, device in enumerate(devices):
            if device.name and device.name.startswith(valid_prefixes):
                print(f"{device_id}     | {device.name} | {device.address}")
                devices_dict[device.address] = [device.name]
